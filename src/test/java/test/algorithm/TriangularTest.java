...
public class TriangularTest extends BaseTest {
    ...
    public void testRandomData() throws Exception {
        ...
        for (int i = 0; i < RANDOM_TEST_COUNT; i++) {
            double theta = PositionUtil.randDouble(0, 360);
            Point p1 = new Point(PositionUtil.randDouble(MIN_RADIUS, MAX_RADIUS), theta, true);
            p1.addOffset(ORIGIN_OFFSET);
            double beta = PositionUtil.randDouble(0, 180);
            theta += beta;
            Point p2 = new Point(PositionUtil.randDouble(MIN_RADIUS, MAX_RADIUS), theta, true);
            p2.addOffset(ORIGIN_OFFSET);
            double alpha = PositionUtil.randDouble(0, 180);
            theta += alpha;
            Point p3 = new Point(PositionUtil.randDouble(MIN_RADIUS, MAX_RADIUS), theta, true);
    ...
    private double runTriangular(int cnt, Point expect, double alpha, double beta, Point p1, Point p2, Point p3) {
        Point ans = PositionUtil.triangular(alpha, beta, p1, p2, p3);
